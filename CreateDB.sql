...
CREATE TABLE IF NOT EXISTS `mydb`.`commodity` (
  ...
  PRIMARY KEY (`commodityID`, `simularCommudity`, `categoryId`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`commodity_has_shop` (
  ...
  CONSTRAINT `fk_commodity_has_shop_user1`
    FOREIGN KEY (`UserId`)
    REFERENCES `mydb`.`user` (`site_siteId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`UserSiteAccess` (
  `site_siteId` INT NOT NULL,
  `user_site_siteId` INT NOT NULL,
  PRIMARY KEY (`site_siteId`, `user_site_siteId`),
  INDEX `fk_site_has_user_user1_idx` (`user_site_siteId` ASC),
  INDEX `fk_site_has_user_site1_idx` (`site_siteId` ASC),
